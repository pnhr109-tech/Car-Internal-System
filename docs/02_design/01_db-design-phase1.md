...
auth.User
  │
  ├── UserProfile（1:1）── Store（null=superuser）
  │
...
|---------|-----|------|------|
| id | BIGINT PK | NO | |
| code | VARCHAR(20) | NO | 識別コード（TSUKUBA / MITO / OYAMA / UTSUNOMIYA / CC / SUPPORT / HQ） |
| name | VARCHAR(50) | NO | 表示名（つくば / 水戸 / 小山 / 宇都宮 / CC / サポート / 本社業務） |
...
### 0-1. user_profiles（ユーザープロファイル）

| カラム名 | 型 | NULL | 説明 |
|---------|-----|------|------|
| id | BIGINT PK | NO | |
| user_id | BIGINT FK→auth.User (unique) | NO | 1:1 |
| store_id | BIGINT FK→stores | YES | 所属店舗（superuserはNULL） |
| role | VARCHAR(20) | NO | ロール（下記CHOICES） |
| employee_number | VARCHAR(20) | YES | 社員番号 |
| is_active_employee | BOOLEAN | NO | 在籍フラグ（default: True） |
...
| value | label | 説明 |
|-------|-------|------|
| general | 一般社員 | 自分のデータのみ変更可 |
| sub_leader | 次席 | 自店舗の全データ変更可 |
| manager | マネージャー | 自店舗の全データ変更可 |
| superuser | 全権限 | 全店舗・全データ（社長・システム管理者） |
...
def has_global_access(self):
    """全店舗データを参照できるか"""
    return self.role == 'superuser' or (self.store and self.store.code == 'HQ')
...
def can_edit_numbers(self):
    ...
    if self.role == 'superuser':
        return True
    if self.store and self.store.code == 'HQ':
        return self.role in ('sub_leader', 'manager')
    return self.role in ('sub_leader', 'manager')

@property
def can_approve(self):
    """承認操作ができるか"""
    return self.role in ('sub_leader', 'manager', 'superuser')
```
...
**channel_type の選択肢:**
```
NAVIKURU       = ナビクル        （申込番号プレフィックス: N）
MYCAR_SCOUT    = マイカースカウト（申込番号プレフィックス: M）
CARVIEW        = カービュー      （申込番号プレフィックス: C）
HP             = ホームページ    （申込番号プレフィックス: H）
WALK_IN        = 来店            （申込番号プレフィックス: W）
REFERRAL       = 紹介            （申込番号プレフィックス: R）
EMAIL          = メール          （申込番号プレフィックス: E）
```

> ナビクル・マイカースカウト・カービューは外部連携で自動取込み予定。
> それ以外は手動入力フォームから登録。手動入力フォームでは上記7チャネルが選択可能（MANUAL廃止）。

**申込番号フォーマット:** `{チャネルプレフィックス}-{YYYYMMDD}-{連番4桁}`
- 例：`N-20260414-0001`（ナビクル）、`H-20260414-0003`（ホームページ）
- 連番は `number_sequences` テーブルで管理（チャネル×日付単位、行ロックで重複防止）
...
### 14. number_sequences（汎用連番管理）

| カラム名 | 型 | NULL | 説明 |
|---------|-----|------|------|
| id | BIGINT PK | NO | |
| sequence_type | VARCHAR(50) | NO | 連番種別（例: 'application_number', 'contract_number'）|
| key | VARCHAR(100) | NO | 連番を区切る単位（例: 'NAVIKURU-20260414'） |
| last_seq | INT UNSIGNED | NO | 最終発行連番（default: 0） |

**ユニーク制約:** `(sequence_type, key)`

> 将来の連番管理（契約番号・承認番号等）も同テーブルで一元管理。
> 発行時は `SELECT FOR UPDATE` で行ロックし重複防止。

---

### 15. contact_histories（取引・連絡履歴）
...
leads/          ← 既存。AssessmentRequest, GmailMessage + 新規モデルを追加
  models.py     ← Customer, Vehicle, VehicleImage, Assessment,
                   ...
                   CustomerBankAccount, NumberSequence を追加
```
