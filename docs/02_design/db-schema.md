...
| 4 | `customers` | Customer | leads | 顧客マスタ |
| 5 | `customer_bank_accounts` | CustomerBankAccount | leads | 顧客口座情報 |
| 6 | `vehicles` | Vehicle | leads | 車両マスタ |
| 7 | `vehicle_images` | VehicleImage | leads | 車両画像 |
| 8 | `number_sequences` | NumberSequence | leads | 申込番号等の連番管理 |
| 9 | `car_assessment_requests` | CarAssessmentRequest | leads | 査定申込（全チャネル統合） |
| 10 | `assessments` | Assessment | leads | 査定・商談 |
| 11 | `assessment_check_items` | AssessmentCheckItem | leads | 査定チェック項目 |
| 12 | `document_type_masters` | DocumentTypeMaster | leads | 書類種別マスタ（将来実装） |
| 13 | `purchase_contracts` | PurchaseContract | leads | 買取契約 |
| 14 | `documents` | Document | leads | 書類・後日品（将来実装） |
| 15 | `identity_documents` | IdentityDocument | leads | 本人確認書類（将来実装） |
| 16 | `ownership_releases` | OwnershipRelease | leads | 所有権解除管理 |
| 17 | `advance_payments` | AdvancePayment | leads | 先払い入金記録 |
| 18 | `contact_histories` | ContactHistory | leads | 取引・連絡履歴 |
...
| フェーズ | 主テーブル | 補助テーブル |
|---|---|---|
| ① 申込受付 | `car_assessment_requests` | `number_sequences` |
